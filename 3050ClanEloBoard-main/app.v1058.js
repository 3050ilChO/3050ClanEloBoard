...
// TST/TSL: merge tier label (갓/킹/...) into winner/runner name cells for mobile "갓 DayDream" style." label (갓/킹/...) into winner/runner name cells for mobile "갓 DayDream" style.
  ...
  function renderBlockTable(tableEl, block, leagueKey){
    ...
    // Remove any row that contains "명예의전당" but is not a season label row.
    try{
      if(String(leagueKey||'').toLowerCase()==='tst' && Array.isArray(data)){
        const _norm=(s)=>String(s||'').replace(/[\u200b-\u200d\ufeff]/g,'').replace(/\xa0/g,' ').replace(/\s+/g,' ').trim();
        data = data.filter(row=>{
          const r = Array.isArray(row)? row : [];
          const txt = _norm(r.join(' '));
          if(/명예의전당/.test(txt) && !/\(\s*시즌\s*\d+\s*\)/.test(txt)) return false;
          return true;
        });
      }
    }catch(_){ }
...
// Build mobile stage-cards for TST/TSL when possible (match TSL screenshot layout)
try{ removeTstTitleRows(tableEl, k); }catch(_){ }
try{
  clearHofStageCards();
  mountStageCardsFromBlock(data, k);
}catch(_){ }
  ...
  function renderStageCardsForMobile(tableEl, blockData, leagueKey){
    ...
    cols.forEach((it, idx)=>{
      ...
      const mkLine = (place, value)=>{
        ...
        badge.textContent = (place==='win') ? '🏆' : '🥈';
        line.appendChild(badge);

        const lab = document.createElement('span');
        lab.className = 'hof-stage-label';
        lab.textContent = (place==='win') ? '우승' : '준우승';
    ...
    // Mark that stage cards exist so CSS can consistently toggle table/cards on mobile/resize
    try{
      const inline = document.getElementById('hofInline');
